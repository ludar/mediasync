...
//	it only checks if sources are dirs. let rsync check access itself
	private static function expand() {
		...
		if (!is_array($map)) {
			return 'map must be an array';
		}
		if (!sizeof($map)) {
			return 'map is not set or empty in conf';
		}
...
//		check source_base
		...
		if ($sBase) {
			if (!is_dir($sBase)) {
				return 'source_base if set must be a directory';
			}
			if (!isAbsolutePath($sBase)) {
				return 'source_base if set must be an absolute path';
			}
		}

//		check destination_base
		$dBase = self::$data['destination_base'] = rtrimDir(arEl(self::$data, 'destination_base', ''));
		if ($dBase && !isAbsolutePath($dBase)) {
			return 'destination_base if set must be an absolute path';
		}
...
//			check source
			if (!isAbsolutePath($s)) {//source_base relative path
				if (!$sBase) {
					return 'either set source_base or use absolute source pathes';
				}
				$s = $sBase . '/' . $s;
			}
...
//			check destination
			if (!isAbsolutePath($d)) {//destination_base relative path
				if (!$dBase) {
					return 'either set destination_base or use absolute destination pathes';
				}
				$d = $dBase . '/' . $d;
			}

			if ($dBase && $d == $dBase) {
				return "destination for '{$s}' is the same as destination_base";
			}
...
//		check sources' inodes for dups
		$dups = array();
		foreach ($map2 as $s => $d) {
			if (($inode = fileinode($s)) === false) {
				return "cant get inode for '{$s}'";
			}
...
//		check destinations for dups
		...
		if (sizeof($dups)) {
			return "at least one destination is referred more than once: " . reset($dups);
		}
