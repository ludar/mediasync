...
function bye($s = '', $c = 1, $silent = false) {
	...
	if (defined('BYE_MAILTO')) {
		@mail(BYE_MAILTO, __WATASHI__ . ' error', $s);
	}
	exit($c);
}
...
function mytask($pidfile, $extra = '') {
	...
	while (1) {
		if (++$c > 8) {
			return 'cant lock pid file';
		}

		if (!file_exists($pidfile)) {
//			$extra is supposed to give a hint about what is this pid file about
			if (file_put_contents($pidfile, $pid . $extra) === false) {
				return 'cant write to pid file';
			}
			sleep(1);
			continue;
		}

		if (($pid2 = file_get_contents($pidfile)) === false) {
			return 'cant read from pid file';
		}
...
//				it looks like the previous worker on this task has died
				if (unlink($pidfile) === false) {
					return 'cant unlink pid file';
				}
				continue;
			}
