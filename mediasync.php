...
if (($e = mytask('/tmp/mediasync-' . md5($host) . 'pid', $host))) {
	bye($e);
}
...
//	here path to rsync is a local one
	$line = sprintf('/usr/bin/rsync %s %s/ %s 2>&1 | tee %s', mergeArgs($rsync_opt), escapeshellarg($s), escapeshellarg($d), $tmp);
